...
module.exports = {
  ...
  parseDate: function parseDate(date, format, culture) {
  ...
  // ### .toDate(date, format, culture)
  // date format helper, done thanks to [jquery.global plugin](https://github.com/jquery/jquery-global)
  // takes a date string and delegates to Globalize.parseDate() and returns its Date object.
  toDate: function toDate(date, format, culture) {
    format = format || config.format;
    culture = culture || config.culture;
    ...
    return jqg.parseDate(date, format, culture);
  },
